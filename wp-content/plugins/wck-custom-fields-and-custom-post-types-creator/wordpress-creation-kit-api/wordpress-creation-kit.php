...
class Wordpress_Creation_Kit{
	...
	function wck_output_meta_content($meta, $id, $fields, $box_args = '' ){	
		...
		if( !empty( $results ) ){
			$list .= apply_filters( 'wck_metabox_content_header_'.$meta , '<thead><tr><th class="wck-number">#</th><th class="wck-content">'. __( 'Cодержание', 'wck' ) .'</th><th class="wck-edit">'. __( 'Изменить', 'wck' ) .'</th><th class="wck-delete">'. __( 'Удалить', 'wck' ) .'</th></tr></thead>' );
			$i=0;
	...
	function wck_output_entry_content( $meta, $id, $fields, $results, $element_id ){
		...
		$list .= '</td>';				
		$list .= '<td style="text-align:center;vertical-align:middle;" class="wck-edit"><a href="javascript:void(0)" class="button-secondary"  onclick=\'showUpdateFormMeta("'.esc_js($meta).'", "'.esc_js($id).'", "'.esc_js($element_id).'", "'.esc_js($edit_nonce).'")\' title="'. __( 'Edit this item', 'wck' ) .'">'. apply_filters( 'wck_edit_button', __('Изменить','wck'), $meta ) .'</a></td>';
		$list .= '<td style="text-align:center;vertical-align:middle;" class="wck-delete"><a href="javascript:void(0)" class="mbdelete" onclick=\'removeMeta("'.esc_js($meta).'", "'.esc_js($id).'", "'.esc_js($element_id).'", "'.esc_js($delete_nonce).'")\' title="'. __( 'Delete this item', 'wck' ) .'">'. apply_filters( 'wck_delete_button', __( 'Удалить', 'wck' ), $meta) .'</a></td>';
