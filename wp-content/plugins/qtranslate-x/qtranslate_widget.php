...
function qtranxf_generateLanguageSelectCodeMy($args = array(), $id='') {
	global $q_config;
	...
	foreach(qtranxf_getSortedLanguages() as $language) {
        ...
        if($language == $q_config['language']) $active="<div class='current_lang'>{$language}</div>";

        $href=qtranxf_convertURL($url, $language, false, true);
        $li.="<div class='lang_item' href='{$href}' data-lang=''>{$language}</div>";
